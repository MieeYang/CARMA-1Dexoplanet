...
subroutine setupatm(carma, cstate, rescale, radplanet, rc)
  ...
  real(kind=f), intent(in)             :: radplanet       !! Planetary radius [cm]
 ...
 ! real(kind=f), parameter :: rmu_const = rmu_0 * (rmu_t0 + rmu_c)
  ...
  real(kind=f), parameter :: rmu_0 = 1.827e-4_f                     
  real(kind=f), parameter :: rmu_t0 = 291.15_f                       
  real(kind=f), parameter :: rmu_c = 120._f         
  real(kind=f), parameter :: rmu_const = rmu_0 * (rmu_t0 + rmu_c)
  ...
  select case(igridh)
    ...
    ! Latitude/Longitude
    case (I_LL)
      xmet(:) = radplanet * DEG2RAD * cos(DEG2RAD * yc(:))    !PETER: REARTH -> radplanet
      ymet(:) = radplanet * DEG2RAD                           !PETER: REARTH -> RPLANET
  ...
  select case(igridv)
	...
	/ (zl(1:NZ) - zl(2:NZP1))) / &
!        (grav(:) * (radplanet/(RPLANET+zc(:)))**2._f * rhoa(:)))
        (grav(:) * rhoa(:)))
