...
!                    write(*,*) "hetgen"
	            ...
	            agnuc = max(0._f,akelvin(iz,igas)/log(supsatl(iz,igas) + 1._f))
  	            if (agnuc .eq. 0._f) then
                      ...
                      osfreq = 1.6e11_f * sqrt(DESORPE*EV2ERG/BK/gwtmol_dif(igas))
	              ...
	              surfcond = fluxmol / osfreq * exp(DESORPE * &
	                EV2ERG / (BK * t(iz)))
...
!                      write(*,*) agnuc, fluxmol, surfcond, zv, r(ibin,igroup),rnuclg(ibin,igroup,ignucto)
!                      write(*,*) deltafg, curvfact, BK, t(iz), -1._f * deltafg*curvfact / (BK * t(iz))
