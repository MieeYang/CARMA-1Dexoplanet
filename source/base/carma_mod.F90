...
contains
  ...
  subroutine CARMA_Initialize(carma, rc, do_cnst_rlh, do_coag, do_detrain, do_fixedinit, &
      ...
      do_pheat, do_pheatatm, dt_threshold, cstick, charge, gsticki, gstickl, tstick, itbnd_pc, &
    ...
    real(kind=f), intent(in), optional  :: charge        !! charge on particle in units of e-/micron, default = 0.0
    ...
    if (present(charge))        carma%f_charge        = charge
...
!      endif
    ...
    do igas = 1, carma%f_NGAS
      ...
      else if (carma%f_gas(igas)%f_icomposition == I_GCOMP_NACL) then
        carma%f_igasnacl = igas
