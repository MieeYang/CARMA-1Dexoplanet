...
!  The data were compiled by Hall (J. Atmos. Sci. 37, 2486-2507, 1980).
  ...
  open(unit=89,file = 'ckernel_frac.txt', status="unknown")
  write(89,'(A114)') '     z[km]           r1[micron]      r1[micron]      cbr[cm3/s]      ccd[cm3/s]      cgr[cm3/s]     ckernel[cm3/s]'
...
!                vfc_2 = (vf(k,i2,j2) + 8.0e-5_f / rhoa_cgs) * zmet(k)          !PETER
  ...
  !                 endif
		...
		!write(*,*) i1,i2,zc(k),ckernel(k,i1,i2,j1,j2)
                  ...
                  if (ckernel(k,i1,i2,j1,j2) .gt. 1e-99_f) then
                    write(89,'(1p7e16.5)') zc(k)/1e5_f,1e4_f*r1, 1e4_f*r2, &
                    cbr, ccd, cgr, ckernel(k,i1,i2,j1,j2)
                  else
                    write(89,'(1p7e16.5)') zc(k)/1e5_f,1e4_f*r1, 1e4_f*r2, 0._f, 0._f, 0._f, 0._f
                  endif
