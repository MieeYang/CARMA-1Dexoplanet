...
!  vertout_sum(:) = 0._f	!PETER
  ...
  do istep = 1, nstep_sed
    ...
    if (gas_switch .eq. 0) then									!PETER
      vertupin_sum(:,igas,1) = vertupin_sum(:,igas,1) + vertupin(:) * dtime / nstep_sed / dz(:)		!PETER
      vertupout_sum(:,igas,1) = vertupout_sum(:,igas,1) + vertupout(:) * dtime / nstep_sed / dz(:)		!PETER
      vertdnin_sum(:,igas,1) = vertdnin_sum(:,igas,1) + vertdnin(:) * dtime / nstep_sed / dz(:)		!PETER
      vertdnout_sum(:,igas,1) = vertdnout_sum(:,igas,1) + vertdnout(:) * dtime / nstep_sed / dz(:)		!PETER
    else											!PETER
      vertupin_sum(:,ibin,ielem) = vertupin_sum(:,ibin,ielem) + vertupin(:) * dtime / nstep_sed / dz(:)	!PETER
      vertupout_sum(:,ibin,ielem) = vertupout_sum(:,ibin,ielem) + vertupout(:) * dtime / nstep_sed / dz(:)	!PETER
      vertdnin_sum(:,ibin,ielem) = vertdnin_sum(:,ibin,ielem) + vertdnin(:) * dtime / nstep_sed / dz(:)	!PETER
      vertdnout_sum(:,ibin,ielem) = vertdnout_sum(:,ibin,ielem) + vertdnout(:) * dtime / nstep_sed / dz(:)	!PETER
    endif 											!PETER
  enddo
