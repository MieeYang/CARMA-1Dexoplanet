...
!  enddo
  ...
  if (do_grow) then
    ...
    if (do_printdiag) then 													!PETER
      ...
      write(lundiag,*) '****************************************'							!PETER
      write(lundiag,'(2A5,A8,10A11)') 'Z', 'BIN', 'ELEM', 'RNUCPE1', 'RHOMPE', 'GROWPE', 'RNUCLG', &      	!PETER
...
!              if (do_print) write(LUNOPRT,*) "newstate::WARNING - Substep failed, retrying with ", ntsubsteps, " substeps."
        ...
        !    if (.not. takeSteps) then											!PETER
              ...
              do ielem = 1, NELEM                                                                                       !PETER
                do ibin = 1, NBIN                                                                                       !PETER	
                  ...
                  rnuclg_tot(iz,ibin,igroup) = rnuclg_tot(iz,ibin,igroup) + sum(rnuclg(ibin,igroup,:))*pc_psolve(iz,ibin,ielem)*dtime !PETER
      ...
      if (do_printdiag) then                                                                                                    !PETER
      do igas = 1, NGAS                                                                                                         !PETER
        write(lundiag,'(3I05,10E13.3)') iz, 0, igas, 0, 0, 0, 0, 0, 0, 0, 0, gasprod_tot(iz,igas), gasprod_tot(iz,igas)          !PETER
      end do                                                                                                                    !PETER
      do ielem = 1, NELEM                                                                                                       !PETER
        do ibin = 1, NBIN                                                                                                       !PETER	
          igroup = igelem(ielem)                                                                                                !PETER
            write(lundiag,'(3I06,10E16.6)') iz, ibin, ielem, rnucpeup_tot(iz,ibin,ielem), &                              !PETER
                                             rhompe_tot(iz,ibin,ielem), growpe_tot(iz,ibin,ielem), &                !PETER
                                             rnuclg_tot(iz,ibin,igroup), growlg_tot(iz,ibin,igroup), &              !PETER
                                             evaplg_tot(iz,ibin,igroup), rnucpe_tot(iz,ibin,ielem), &               !PETER
                                             evappe_tot(iz,ibin,ielem), 0,   &    !PETER
                                             rnucpeup_tot(iz,ibin,ielem) + rhompe_tot(iz,ibin,ielem) + growpe_tot(iz,ibin,ielem) - rnuclg_tot(iz,ibin,igroup) - growlg_tot(iz,ibin,igroup) - evaplg_tot(iz,ibin,igroup) + rnucpe_tot(iz,ibin,ielem) + evappe_tot(iz,ibin,ielem)
        end do                                  !PETER
      end do                                    !PETER
      end if                                    !PETER
...
!    write(*,*) 'NETFLUX'
!    write(*,*) (rnucpeup_tot(33,33,2) + rhompe_tot(33,33,2) + &
!growpe_tot(33,33,2) - rnuclg_tot(33,33,2) - &
!growlg_tot(33,33,2) - evaplg_tot(33,33,2) + &
!rnucpe_tot(33,33,2) + evappe_tot(33,33,2))
!    write(*,*) ''
