...
subroutine setupvf_std_shape(carma, cstate, j, rc)
  ...
  real(kind=f)            :: omega, beta, kappa, psi, falltime
...
!      f1  = eshape(j)**(-ONE/3._f) * (xcc + 2._f*xa) / 3._f
  ...
  open(unit=89,file = 'fallspeed_agg.txt', status="unknown")
  write(89,'(A139)') '     z[km]     r[micron] p_rho[g/cm3] a_rho[g/cm3]  g[cm/s2]     fslip     vis[Poise] vfall[cm/s]     Re       omega     lambda[cm]      Kn'   
...
!        r_shape = r_wet(k,i,j) * eshape(j)**(-ONE/3._f)
      ...
      vf(k,i,j) = ((2._f / 9._f) * (rhop_wet(k,i,j)-rhoa_cgs) * &
!        r_shape**2 * grav(k) * (RPLANET/(RPLANET+zc(k)))**2._f * bpm(k,i,j) / (f1 * rmu(k)) - winds(k)
        r_shape**2 * grav(k) * bpm(k,i,j) / (f1 * rmu(k)))*omega - winds(k)
!      vf(k,i,j) = vf(k,i,j)*omega
      re(k,i,j) = 2._f * rhoa_cgs * r_shape * (vf(k,i,j)+winds(k)) / rmu(k)
      re_keepsign = re(k,i,j) / abs(re(k,i,j))
      re(k,i,j) = abs(re(k,i,j))   
      
      write(89,'(1p12e12.3)') zc(k)/1e5_f,1e4_f*r_shape,rhop_wet(k,i,j),rhoa_cgs, &
                 grav(k),bpm(k,i,j),rmu(k),vf(k,i,j),re(k,i,j), &
                 omega,rmfp,rkn
      !write(*,*) 'iz=',k,'ibin=',i,'dz=',dz(k),'vf=',vf(k,i,j),'dz/vf=',dz(k)/vf(k,i,j)
      if (i .eq. 6) falltime = falltime + dz(k)/vf(k,i,j)
...
!            x = log10(2._f * rfix / eshape(j))
        ...
        vf(k,i,j) = re(k,i,j) * re_keepsign * rmu(k) / (2._f * r_shape * rhoa_cgs) - winds(k)
