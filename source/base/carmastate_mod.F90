...
contains
  ...
  subroutine CARMASTATE_Create(cstate, carma_ptr, time, dtime, NZ, igridv, igridh,  &
      ...
      ekz, ftopp, fbotp, pctop, pcbot, gctop, gcbot, ftopg, fbotg, met, prod, prodgas)        !PETER
...
!      lat, lon, xc, dx, yc, dy, zc, zl, p, pl, t, rc, qh2o, relhum)
      lat, lon, xc, dx, yc, dy, zc, zl, p, pl, t, wtmol_air, grav, rplanet, rc, qh2o, relhum, winds, ekz, met) 				!PETER
    type(carmastate_type), intent(inout)    :: cstate      !! the carma state object
    type(carma_type), pointer, intent(in)   :: carma_ptr   !! (in) the carma object
    real(kind=f), intent(in)                :: time        !! the model time [s]
    real(kind=f), intent(in)                :: dtime       !! the timestep size [s]
	...
	cstate%f_carma%f_do_grow .or. cstate%f_carma%f_do_drydep) then
      ...
      if (cstate%f_carma%f_do_coag) then
        allocate( &
          ...
          cstate%f_coagloss(NZ,NBIN,NGROUP), &
...
!    if (ielem == 2 .and. ibin == 33) then
!      write(*,*) 'CARMASTATE: GetBin ielem 2 ibin 33'
!      write(*,*) mmr(33) , cstate%f_pc(33, 33, 2) 
!    end if 
...
!    where (cstate%f_pc(:, ibin, ienconc) <= SMALL_PC) mmr(:) = 0.0_f
	...
	/ (cstate%f_xmet(:)*cstate%f_ymet(:)*cstate%f_zmet(:))
      ...
      if (cstate%f_carma%f_do_vtran) then
        ...
        if (present(ekz))           ekz(:)             = cstate%f_ekz(:)                     !PETER
      else
  ...
  subroutine CARMASTATE_GetDiag(cstate, rc, vertpartflux, vertgasflux, gasprod_tot, rnucpeup_tot, rhompe_tot, &
                                growpe_tot, rnuclg_tot, growlg_tot, evaplg_tot, rnucpe_tot, evappe_tot, coagprod, coagloss)      !PETER
    ...
    real(kind=f), optional, intent(out)   :: coagprod(cstate%f_NZ,cstate%f_carma%f_NBIN,cstate%f_carma%f_NELEM)       !! Production rate from coagulation		!PETER
    real(kind=f), optional, intent(out)   :: coagloss(cstate%f_NZ,cstate%f_carma%f_NBIN,cstate%f_carma%f_NGROUP)       !! Loss rate from coagulation		!PETER
    ...
    if (present(coagprod))  coagprod(:,:,:)  = cstate%f_coagprod(:,:,:)                                         !PETER
    if (present(coagloss))  coagloss(:,:,:)  = cstate%f_coagloss(:,:,:)                                         !PETER
...
!    if (ielem == 2 .and. ibin == 33) then
!      write(*,*) 'CARMASTATE: SetBin ielem 2 ibin 33'
!      write(*,*) mmr(33) , cstate%f_pc(33, 33, 2)     
!    end if 
