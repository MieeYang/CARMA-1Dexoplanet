...
!! @author Eric Jensen
!! @version Dec-1996
subroutine versol(carma, cstate, cvert, vtrans, itbnd, ibbnd, ftop, fbot, cvert_tbnd, cvert_bbnd, &
...
!          vertadvd(k), vertadvu(k+1), cour, uc
  ...
  if(( igridv .eq. I_SIG ) .or. ( igridv .eq. I_HYBRID )) then
    ...
    if( itbnd .eq. I_FIXED_VEL ) dl(NZ) = dl(NZ) + max(0._f,vtrans(NZP1))*cvert(NZ)
    if( ibbnd .eq. I_FIXED_VEL ) dl(1) = dl(1) - vtrans(1)*cvert(1)
  endif
  ...
  if (gas_switch .eq. 0) then
    ...
    elseif (itbnd .eq. I_FIXED_VEL) then
      gflux(NZP1,igas) = max(0._f,vtrans(NZP1))*cvert(NZ)
    else
      gflux(NZP1,igas) = cvert(NZ)*(vertadvu(NZP1)+vertdifu(NZP1)) - ctempu(NZ)*(vertadvd(NZP1)+vertdifd(NZP1))
    endif
    if (ibbnd .eq. I_FLUX_SPEC) then
      gflux(1,igas) = fbot
    elseif (ibbnd .eq. I_FIXED_VEL) then
      gflux(1,igas) = vtrans(1)*cvert(1)
    endif
  else
    do k = 1,NZ
    ...
    elseif (itbnd .eq. I_FIXED_VEL) then
      pflux(NZP1,ibin,ielem) = max(0._f,vtrans(NZP1))*cvert(NZ)
    else
      pflux(NZP1,ibin,ielem) = cvert(NZ)*(vertadvu(NZP1)+vertdifu(NZP1)) - ctempu(NZ)*(vertadvd(NZP1)+vertdifd(NZP1))
    endif
    if (ibbnd .eq. I_FLUX_SPEC) then
      pflux(1,ibin,ielem) = fbot
    elseif (ibbnd .eq. I_FIXED_VEL) then
      pflux(1,ibin,ielem) = vtrans(1)*cvert(1)
    endif
  endif
  !write(*,*) ibin,ielem,pflux(NZP1,ibin,ielem),ftop,ctempu(NZ)
...
!  if ((ibin .eq. 1) .and. (ielem .eq. 1)) then
    !write(*,*) cvert(12), cvert(13),cvert(14), cvert(15)
!    write(*,*) '10', vertdifu(10), vertdifd(10)
!    write(*,*) '11', vertdifu(11), vertdifd(11)
!    write(*,*) '12', vertdifu(12), vertdifd(12)
!    write(*,*) '13', vertdifu(13), vertdifd(13)
!    write(*,*) '14', vertdifu(14), vertdifd(14)
!    write(*,*) '15', vertdifu(15), vertdifd(15)
!    write(*,*) '16', vertdifu(16), vertdifd(16)
!    write(*,*) '17', vertdifu(17), vertdifd(17)
!    write(*,*) '18', vertdifu(18), vertdifd(18)
!  endif
