...
!      rlhm(k,igash2so4) = rlhe(k, igash2o)
    ...
    if (igaskcl .ne. 0) then
      ! Diffusivity
      rhoa_cgs = rhoa(k) / (xmet(k) * ymet(k) * zmet(k))
      diffus(k,igaskcl) = 5._f / (16._f * AVG * (0.5_f*(COLDIA_ATM+COLDIA_KCL))**2_f * rhoa_cgs * COLINT) * &
    ...
    ! Properties for NaCl
    if (igasnacl .ne. 0) then
      ! Diffusivity
      rhoa_cgs = rhoa(k) / (xmet(k) * ymet(k) * zmet(k))
      diffus(k,igasnacl) = 5._f / (16._f * AVG * (0.5_f*(COLDIA_ATM+COLDIA_NACL))**2_f * rhoa_cgs * COLINT) * &
  	sqrt(RGAS * t(k) * wtmol_air(k) * (WTMOL_NACL + wtmol_air(k)) / (2._f * PI * WTMOL_NACL)) 
      rlhe(k,igasnacl) = TCOEFF_NACL * log(10._f) * RGAS / WTMOL_NACL * t(k)**2._f  / (t(k) - 82.638_f)**2._f ! Derived from Lee et al. (2018)
      rlhm(k,igasnacl) = rlhe(k, igasnacl)
    end if
    ...
    if (igaszns .ne. 0) then
      ! Diffusivity
      rhoa_cgs = rhoa(k) / (xmet(k) * ymet(k) * zmet(k))
      diffus(k,igaszns) = 5._f / (16._f * AVG * (0.5_f*(COLDIA_ATM+COLDIA_ZN))**2_f * rhoa_cgs * COLINT) * &
    ...
    if (igasna2s .ne. 0) then
      ! Diffusivity
      rhoa_cgs = rhoa(k) / (xmet(k) * ymet(k) * zmet(k))
      diffus(k,igasna2s) = 0.5_f * 5._f / (16._f * AVG * (0.5_f*(COLDIA_ATM+COLDIA_NA))**2_f * rhoa_cgs * COLINT) * &
    ...
    if (igasmns .ne. 0) then
      ! Diffusivity
      rhoa_cgs = rhoa(k) / (xmet(k) * ymet(k) * zmet(k))
      diffus(k,igasmns) = 5._f / (16._f * AVG * (0.5_f*(COLDIA_ATM+COLDIA_MN))**2_f * rhoa_cgs * COLINT) * &
    ...
    if (igascr .ne. 0) then
      ! Diffusivity
      rhoa_cgs = rhoa(k) / (xmet(k) * ymet(k) * zmet(k))
      diffus(k,igascr) = 5._f / (16._f * AVG * (0.5_f*(COLDIA_ATM+COLDIA_CR))**2_f * rhoa_cgs * COLINT) * &
    ...
    if (igasfe .ne. 0) then
      ! Diffusivity
      rhoa_cgs = rhoa(k) / (xmet(k) * ymet(k) * zmet(k))
      diffus(k,igasfe) = 5._f / (16._f * AVG * (0.5_f*(COLDIA_ATM+COLDIA_FE))**2_f * rhoa_cgs * COLINT) * &
    ...
    if (igasmg2sio4 .ne. 0) then
      ! Diffusivity
      rhoa_cgs = rhoa(k) / (xmet(k) * ymet(k) * zmet(k))
      diffus(k,igasmg2sio4) = 0.5_f *  5._f / (16._f * AVG * (0.5_f*(COLDIA_ATM+COLDIA_MG))**2_f * rhoa_cgs * COLINT) * &
    ...
    if (igastio2 .ne. 0) then
      ! Diffusivity
      rhoa_cgs = rhoa(k) / (xmet(k) * ymet(k) * zmet(k))
      diffus(k,igastio2) = 5._f / (16._f * AVG * (0.5_f*(COLDIA_ATM+COLDIA_TIO2))**2_f * rhoa_cgs * COLINT) * &
    ...
    if (igasal2o3 .ne. 0) then
      ! Diffusivity
      rhoa_cgs = rhoa(k) / (xmet(k) * ymet(k) * zmet(k))
      diffus(k,igasal2o3) = 0.5_f * 5._f / (16._f * AVG * (0.5_f*(COLDIA_ATM+COLDIA_AL))**2_f * rhoa_cgs * COLINT) * &
