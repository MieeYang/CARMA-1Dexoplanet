...
!  real(kind=f)                   :: surf_tens(NZ)  ! surface tension of non-H2O particles
 ...
 !     surfctwa(:) = 70.03_f - 0.039_f*( t(:) - T0 )
    ...
    ! condensing gas is H2SO4                    
      ...
      do k = 1, NZ  
        surf_tens(k,igas) = sulfate_surf_tens(carma, wtpct(k), t(k), rc)
        rho_part = sulfate_density(carma, wtpct(k), t(k), rc)
        akelvin(k, igas) = 2._f * gwtmol(igas) * surf_tens(k,igas) / (t(k) * rho_part * RGAS)
      ...
      do k = 1, NZ  
        surf_tens(k,igas) = 60.8_f ! http://cameochemicals.noaa.gov/chris/SXX.pdf, Fanelli 1950
        rho_part = 1.96_f ! http://periodictable.com/Elements/016/data.html
        akelvin(k, igas) = 2._f * gwtmol(igas) * surf_tens(k,igas) / (t(k) * RHO_SX * RGAS)
      ...
      ! Calculate Kelvin curvature factor for polysulfur interactively with temperature:
      do k = 1, NZ  
        surf_tens(k,igas) = 160.4_f - 0.07_f*(t(k)-273.15_f) ! http://www.kayelaby.npl.co.uk/general_physics/2_2/2_2_5.html 
!        surf_tens(k) = 4._f*(160.4_f - 0.07_f*t(k)) ! http://www.kayelaby.npl.co.uk/general_physics/2_2/2_2_5.html 
        akelvin(k, igas) = 2._f * gwtmol(igas) * surf_tens(k,igas) / (t(k) * RHO_KCL * RGAS)
        
        ! Not doing condensation of h2So4 on ice, so just set it to the value
        ! for water vapor.
        akelvini(k, igas) = akelvin(k, igas)
      end do   
    else if (igas .eq. igasnacl) then
      ! Calculate Kelvin curvature factor for polysulfur interactively with temperature:
      do k = 1, NZ  
        surf_tens(k,igas) = 171.5_f - 0.0719_f*(t(k)-273.15_f) ! Janz 1969 (Molten Salts: Volume 2)
        akelvin(k, igas) = 2._f * gwtmol(igas) * surf_tens(k,igas) / (t(k) * RHO_NACL * RGAS)
      ...
      do k = 1, NZ  
        ...
        surf_tens(k,igas) = 860._f ! Zhang et al. 2003, J. Phys. Chem. B 2003, 107, 13051-13060
        akelvin(k, igas) = 2._f * gwtmol(igas) * surf_tens(k,igas) / (t(k) * RHO_ZNS * RGAS)
      ...
      do k = 1, NZ  
        ...
        surf_tens(k,igas) = 1033._f ! Graham Lee's calculations, estimated from enthalpy data  
        !surf_tens(k) = 50._f ! DON'T KNOW
        akelvin(k, igas) = 2._f * gwtmol(igas) * surf_tens(k,igas) / (t(k) * RHO_NA2S * RGAS)
      ...
      do k = 1, NZ  
        !surf_tens(k) = 50._f ! DON'T KNOW
        !surf_tens(k) = 160.4_f - 0.07_f*t(k) ! Assume same as KCl  
        surf_tens(k,igas) = 2326._f ! Graham Lee's calculations, estimated from enthalpy data  
        akelvin(k, igas) = 2._f * gwtmol(igas) * surf_tens(k,igas) / (t(k) * RHO_MNS * RGAS)
      ...
      ! Calculate Kelvin curvature factor for polysulfur interactively with temperature:
      do k = 1, NZ  
        surf_tens(k,igas) = 1642._f - 0.2_f * ( t(k) - 2133.15_f ) ! http://www.kayelaby.npl.co.uk/general_physics/2_2/2_2_5.html 
        akelvin(k, igas) = 2._f * gwtmol(igas) * surf_tens(k,igas) / (t(k) * RHO_CR * RGAS)
      ...
      ! Calculate Kelvin curvature factor for polysulfur interactively with temperature:
      do k = 1, NZ  
        surf_tens(k,igas) = 1862._f - 0.39_f * ( t(k) - 1803.15_f ) ! http://www.kayelaby.npl.co.uk/general_physics/2_2/2_2_5.html 
        akelvin(k, igas) = 2._f * gwtmol(igas) * surf_tens(k,igas) / (t(k) * RHO_FE * RGAS)
      ...
      ! Calculate Kelvin curvature factor for polysulfur interactively with temperature:
      do k = 1, NZ  
        !surf_tens(k,igas) = 1280._f ! de Leeuw et al. (2000)
        surf_tens(k,igas) = 436._f ! Kozasa et al. (1989) (melt)
        akelvin(k, igas) = 2._f * gwtmol(igas) * surf_tens(k,igas) / (t(k) * RHO_MG2SIO4 * RGAS)
      ...
      do k = 1, NZ  
        ...
        surf_tens(k,igas) = 535.124_f - 0.04396_f * t(k) ! Lee et al. (2015)
        akelvin(k, igas) = 2._f * gwtmol(igas) * surf_tens(k,igas) / (t(k) * RHO_TIO2 * RGAS)
      ...
      ! Calculate Kelvin curvature factor for polysulfur interactively with temperature:
      do k = 1, NZ  
        !surf_tens(k,igas) = 970._f ! Tavakoli et al. (2013)
        surf_tens(k,igas) = 690._f ! Kozasa et al. (1989) (melt)
        akelvin(k, igas) = 2._f * gwtmol(igas) * surf_tens(k,igas) / (t(k) * RHO_AL2O3 * RGAS)
      ...
      do k = 1, NZ  
        ...
        surf_tens(k,igas) = max(27.77_f*(1._f - t(k)/132.92_f)**1.126_f,1e-5_f) ! Sprow & Prausnitz (1965)
        akelvin(k, igas) = 2._f * gwtmol(igas) * surf_tens(k,igas) / (t(k) * RHO_CO * RGAS)
