...
!  real(kind=f)            :: rhoa_cgs, vg, rmfp, rkn, expon
  ...
  open(unit=89,file = 'fallspeed_sph.txt', status="unknown")
  write(89,'(A129)') '     z[km]     r[micron] p_rho[g/cm3] a_rho[g/cm3]  g[cm/s2]     fslip     vis[Poise] vfall[cm/s]     Re       lambda[cm]      Kn'   
...
!      vf(k,i,j) = (ONE * 2._f / 9._f) * rhop_wet(k,i,j) * r_wet(k,i,j)**2 * GRAV * (RPLANET/(RPLANET+zc(k)))**2._f * bpm(k,i,j) / rmu(k)
      vf(k,i,j) = (ONE * 2._f / 9._f) * (rhop_wet(k,i,j)-rhoa_cgs) * r_wet(k,i,j)**2 &
...
!                      sqrt( 8._f * rhop_wet(k,i,j) * r_wet(k,i,j) * grav(k) * (RPLANET/(RPLANET+zc(k)))**2._f / &              !PETER
                      sqrt( 8._f * (rhop_wet(k,i,j)-rhoa_cgs) * r_wet(k,i,j) * grav(k) / &              !PETER
...
!        write(*,*) 'k=',k,'r=',r_wet(k,i,j),'t=',t(k),'vfall=',vf(k,i,j)
      ...
      write(89,'(1p11e12.3)') zc(k)/1e5_f,1e4_f*r_wet(k,i,j),rhop_wet(k,i,j),rhoa_cgs, &
                 grav(k),bpm(k,i,j),rmu(k),vf(k,i,j),re(k,i,j),rmfp,rkn
